...
// Aquí puedes guardar la orden en la DB
...
// Guardar la orden (ejemplo simple, necesitarías tabla de órdenes)
$stmt = $conexion->prepare("INSERT INTO ordenes (nombre, email, total, metodo_pago) VALUES (?, ?, ?, ?)");
$stmt->bind_param("ssds", $nombre, $email, $total, $metodo_pago);
$stmt->execute();
...
<head>
  ...
  <link rel="stylesheet" href="css/estilosprocesarpago.css">
...
<div class="contenedor-confirmacion">
    ...
    <p>Hemos recibido tu pedido y el total fue <strong>$<?= number_format($total, 2) ?></strong>.</p>
    <p>Te enviaremos un correo de confirmación a <strong><?= htmlspecialchars($email) ?></strong>.</p>
